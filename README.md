...
Limitations: as a low-cost hardware project, this device will not directly compete with the existing LEGO(R)-integrated solutions on the market. Only ROMs for the original Game Boy (.gb) are supported (not Game Boy Color or Game Boy Advance). There are fewer hardware features compared to the others (no headphone socket, no volume/contrast knobs) and a compromise in display quality (for scrolling and movement) as a result of the non-integer scaling. There are also simplifications in the LEGO(R) build to accomodate the device; the display bezel is a 3D-printed part, instead of fitting into or behind the brick-built display window. The power is supplied from 2x AA cells, rather than a rechargeable lithium battery.
